...
export default function HDVideoCallActions(props) {
    ...
    const toggleLocalVideoStateIconName = isLocalVideoEnabled
        ? 'videocam'
        : 'videocam-outline';
    const toggleLocalAudioStateIconName = isLocalAudioEnabled
        ? 'mic'
        : 'mic-off-outline';
    ...
    useEffect(() => {
        const connectedToRoomListener = hdEventEmitter.addListener(
            'connectedToRoom',
            event => {
                const videoCallStatus = _.isEmpty(event.participants)
                    ? 'waiting'
                    : 'in-progress';
        ...
        const participantConnectionEventListener = hdEventEmitter.addListener(
    ...
    async function flipCameraDirection() {
        ...
        setCurrentCameraDirection(
            currentCameraDirection === 'back' ? 'front' : 'back',
        );
    }
    ...
    return (
        <TouchableWithoutFeedback
            ...
            style={{zIndex: 2, elevation: 3, position: 'absolute'}}>
            <View
                    ...
                    <View
                        ...
                        <View
                            ...
                            <ToggleAction
                                ...
                                disabled={areControlsHidden}
                                size={54}
                                icon={toggleLocalVideoStateIconName}
                            />
                            <ToggleAction
                                onPress={toggleLocalAudioState}
                                disabled={areControlsHidden}
                                size={54}
                                icon={toggleLocalAudioStateIconName}
                            />
                            <ToggleAction
                                onPress={flipCameraDirection}
                                disabled={areControlsHidden}
                                size={54}
                                icon={'ios-camera-reverse'}
                            />
                        </View>
                    </View>
                    <Animated.View
                        style={{height: Animated.multiply(controlsOpacity, 64)}}
                    />
                </Animated.View>
            </View>
        </TouchableWithoutFeedback>
        ...
        <TouchableOpacity
            ...
            <View
                style={{
                    ...
                    width: props.size,
                    borderRadius: props.size,
                    backgroundColor: props.color || alpha('#444444', 0.9),
                }}
            />
            <Icon
                name={props.icon}
