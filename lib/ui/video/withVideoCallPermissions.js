...
export default function withVideoCallPermissions(WrappedComponent) {
    return function VideoCallPermissionsGate(props) {
        ...
        function doShowGate() {
            Animated.timing(opacityRef.current, {toValue: 1, duration: 800, useNativeDriver: true}).start();
        }
        ...
        function goToSettings() {
            ...
            const checkListener = AppState.addEventListener('change', appState => {
                if (appState === 'active') {
                    checkPermissions().catch((error) => console.warn('[goToSettings:checkPermissions]', error));
