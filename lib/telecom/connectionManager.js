...
export async function registerIncomingVideoCall(
    callUUID,
    videoRoomSID,
    consultationID,
    caller,
            ...
            return new Promise(resolve => {
                setTimeout(() => {
                    ...
                    if (incomingCall.status === 'incoming') {
                        tryCancelVideoCallNotification(
                            incomingCall.videoRoomSID,
                        ).finally(resolve);
                    }
                }, 45000);
            });
...
export function handleIncomingVideoCallStarted(videoRoomSID) {
    ...
    if (!call) {
        console.warn(
            `[handleIncomingVideoCallStarted] cannot start ${videoRoomSID}: no call found`,
        );
        return;
    }
...
export async function endVideoCall(videoRoomSID) {
    ...
    if (call.status === 'completed') {
        console.info(
            `[endConsultationVideoCall:${videoRoomSID}]: call has already been completed`,
        );
        return;
    }
...
export async function registerPushKitCall(notification) {
    ...
    await registerIncomingVideoCall(
        callUUID,
        videoRoomSID,
        consultationID,
        caller,
